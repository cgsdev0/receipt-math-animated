...
module rec Canvas : sig
  ...
  (** Create a 2d rendering context that is attached to this canvas. Set
      [~will_read_frequently] to true if you intend on using [get_image_data] on the
      returned context. *)
  val ctx2d : ?will_read_frequently:bool -> t -> Ctx2d.t

  (** Duplicate this canvas by drawing this canvas onto a new canvas. Set
      [~will_read_frequently] to true if you intend on using [get_image_data] on the
      returned context. *)
  ...
  (** Create a new canvas by copying the pixel data from an image Set
      [~will_read_frequently] to true if you intend on using [get_image_data] on the
      returned context. *)
...
and Ctx2d : sig
  ...
  (** draws another canvas onto this context
      ...
      - [sw], [sh] specify the "source" width and height defaults to the width and height
        of the canvas
      - [w], [h] specify the width and height of the rectangle to be drawn defaults to the
        width and height of the canvas if [w, h] don't match [sw, sh], then the image will
        be squished
  ...
  (** Copies the pixel data from an [Image_data.t] directly into this context.
      ...
      The "dirty" optional parameters define the region _within the Image_data.t_ that
      will be copied. [dirty_x] and [dirty_y] do _not_ adjust the position that the image
      will be drawn at.
...
and Image : sig
  ...
  (** loads an image from the given url. The image will be passed to [on_load] when it
      finishes loading. *)
  val of_url : string -> on_load:(t -> unit) -> unit

  (** Creates an image from a canvas2d context. The image will be passed to [on_load] when
      it finishes loading. *)
